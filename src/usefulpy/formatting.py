...
def a_an(nextword):
    ...
    for vowel in vowels:
        if nextword.startswith(vowel) and vowel != 'u':
            return 'an'
    return 'a'
...
def punctuate(n, witha='.'):
    ...
    if witha not in endingpuncuation:
        raise ValueError(f'{witha} not in {endingpuncuation}')
...
def translate(string, translater):
    ...
    for old, new in translater.items():
...
def Fix_stolong(text):
    '''replace certain instances of s with long s according to older spelling
rules'''
    if text == '':
        return ''
    if 's' not in text:
...
written-out greek-letters or numbers. Et cetera'''
    ...
    for word in loop:
        ...
        if word in numbers:
            listtext[index] = numbers[word]
        if word in greek_letters:
            listtext[index] = greek_letters[word]
        index += add
    runtext = []
    is_prevnum = False
    prevnum = 0
            ...
            if is_prevnum:
                if is_postpoint:
                    ...
                    if is_dozen:
                        ...
                        hundredsgroup = 0
                    appender = _validation.trynumber(runcount+hundredsgroup)
                    runtext.append(str(appender))
                    runcount = 0
                    hundredsgroup = 0
                is_prevnum = False
                prevnum = 0
...
def write(text):
    ...
    if ' ' in text:
        return ' '.join(map(write, text.split()))
    if '.' not in text:
        num = _validation.tryint(text)
    else:
        num = _validation.tryfloat(text)
    if type(num) is int:
        numstr = str(num)
        nicenum = ''
        for count, letter in enumerate(numstr):
            nicenum += letter
            if (len(numstr)-(count+1)) % 3 == 0 and (len(numstr) != (count+1)):
                nicenum += ','
        return nicenum
...
def _TensGroup(num):
    ...
    if str(num) in _OddOnes:
        return _OddOnes[str(num)]
    else:
        if str(num)[0] == '0':
            return _Digit[(str(num)[1])]
        if str(num)[1] == '0':
            return _tensPlace[(str(num)[0])]
...
def _HundredsGroup(num):
    ...
    if int(num) == 0:
        ...
        return _Digit[str(num)]
    elif len(str(num)) == 2:
        return _TensGroup(num)
    elif len(str(num)) == 3:
        if str(num)[1:] == '00':
            return _Digit[(str(num)[0])] + ' hundred'
...
nine hundred ninety nine septillion nine hundred ninety nine hexillion
nine hundred ninety nine quitillion nine hundred ninety nine quatrillion
...
'one hundred\''''
    ...
    for x in range(1, Group + 1):
        ...
        if str(group) in LargeGroups:
            if not PrevZero:
                # Prevents extra words, like thousand in one million thousand
            ...
            raise ValueError('Number is bigger that '+'9'*30)
...
def Syllables(word: str):
    ...
    try:
        word = ComposeNumber(int(float(word)))
    except Exception:
        pass
    if ' ' in word:
        # If sentence is input, counts syllables for each word in sentence
        words = word.split()
        SyllCount = 0
        for word in words:
    ...
    if word in special:  # Some english words are weird.
        return special[word]

    Base = ''  # Consonants and Vowels
    SyllCount = 1  # Count of syllables
    endings = ['VCCV', 'VCV', 'VCCCV', 'VCCCCV',
               'VCY', 'VCCY', 'VCCCY', 'VCCCCY', 'VYV']
    # Posible reasons to split syllables, Y is sometimes a
    # consonant and sometimes a vowel.
...
_allignments = {'center': str.center, 'left': str.ljust,
                'right': str.rjust, 'justify': justify}
...
class multline(object):
    ...
    def __getitem__(self, get):
        def unpack(value, default):
            return value if type(value) is tuple else (value, default)
        ...
        if type(get) is slice:
            if tuple in map(type, (get.start, get.stop, get.step)):
                ...
                return self.getcolumn(slice(start1, stop1, step1)).getrow(
                    slice(start2, stop2, step2))
    ...
    def extend(self, other, center=None):
        ...
        for num in range(newheight+1):
            ...
            try:
                ...
                first = firsts[num-offset1]
            except Exception: first = ' '*other.width
            try:
                if num-offset2<0: raise Exception
                second = seconds[num-offset2]
            except Exception: second = ' '*self.width
            new.append(first+second)
        return multline(*new)'''
        return NotImplemented
