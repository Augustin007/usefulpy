...
__all__ = ('S', 'acos', 'acosh', 'acot', 'acoth', 'acsc', 'acsch', 'asec',
           ...
           'ln', 'log', 'log1p', 'log2', 'mathfunc', 'mathfunction',
           'polynomial', 'sec', 'sech', 'sigmoid', 'sin', 'sinh', 'sqrt',
           'square', 'tan', 'tanh', 'tesser', 'x', 'y', 'z')
...
def _comp_derive(n, var, k):
    ...
    # logging computation
    log = f'Computing {k} partial of {safe_str(n)} with respect to {var.name}'
    logging.info(log)
...
class cas_variable:
    ...
    def __neg__(self):
        '''Return -self'''
        try:
            return self._math_return(mul_expression((-1, self)))
        except Exception as error:
            logging.debug(f'{error.__class__.__name__}: {error.args[0]}')
    ...
    def __add__(self, other):
        '''Return self+other'''
        try:
            return self._math_return(add_expression((self, other)))
        except Exception as error:
            logging.debug(f'{error.__class__.__name__}: {error.args[0]}')
            return NotImplemented
    ...
    def __mul__(self, other):
        '''Return self*other'''
        try:
            return self._math_return(mul_expression((self, other)))
        except Exception as error:
            logging.debug(f'{error.__class__.__name__}: {error.args[0]}')
            return NotImplemented
    ...
    def reciprocal(self):
        '''Return 1/self'''
        try:
            return self._math_return(pow_expression(self, -1))
        except Exception as error:
            logging.debug(f'{error.__class__.__name__}: {error.args[0]}')
            return NotImplemented
    ...
    def __pow__(self, other):
        '''Return self**other'''
        try:
            return self._math_return(pow_expression(self, other))
        except Exception as error:
            logging.debug(f'{error.__class__.__name__}: {error.args[0]}')
            return NotImplemented
    ...
    def __rpow__(self, other):
        '''Return other**self'''
        try:
            return self._math_return(pow_expression(other, self))
        except Exception as error:
            logging.debug(f'{error.__class__.__name__}: {error.args[0]}')
            return NotImplemented
...
def comp_extract(x):
    '''Removes mathfunc in favour of its cas composition'''
...
def expression_function(func):
    ...
    def wrap(*args):
            ...
            for n, a in enumerate(args):
                ...
                if ap:
                    if len(exchange.prime[n]) == 2:
                        prime_func = exchange.prime[n][0]
                        prime_args = exchange.prime[n][1]
                        fp = prime_func(*get_args(args, prime_args))
                        fp = fp.composition
                    else:
                        fp = exchange.prime[n]
        ...
        if exchange.format:
            string = exchange.format
            for n, arg in enumerate(args):
                string = string.replace(f'<{n}>', f'({safe_str(arg)})')

        else:
            string = func.__name__+'('
            string += ', '.join(map(safe_str, args))
            string += ')'
        ...
        _internal_exchange.view_str = string
